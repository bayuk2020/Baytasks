    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-lg max-h-[90vh] overflow-y-auto glass-strong rounded-2xl p-6 relative"
        >
          {/* CLOSE */}
          <button
            onClick={onClose}
          ...
          <div className="mt-5 space-y-4">
            ...
            {/* EMOJI */}
            <div>
              <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Emoji</label>
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {EMOJIS.map((e) => (
                  <button
                    key={e}
                    onClick={() => setEmoji(e)}
                    className={`h-9 w-9 rounded-lg border grid place-items-center text-lg transition ${
                      emoji === e ? "border-[var(--neon)] bg-accent" : "border-border hover:bg-accent"
                    }`}
                  >
                    {e}
                  </button>
            ...
            {/* COLORS */}
            <div>
              <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Aura Color</label>
              <div className="mt-1.5 flex gap-2">
                {COLORS.map((c) => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
                    className={`h-8 w-8 rounded-full border-2 transition ${color === c ? "scale-110" : "border-transparent"}`}
                    style={{
                      background: SWATCH[c],
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Frequency</label>
                <select
                  value={frequency}
              ...
              <div>
                <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">XP Reward</label>
                <input
                  type="number"
                  min={1}
                  max={200}
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            <button onClick={onClose} className="rounded-lg px-4 py-2 text-sm hover:bg-accent">
              Cancel
            </button>
            <button
              onClick={submit}
