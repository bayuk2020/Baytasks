...
function Column({ id, title, tasks, onOpen, onAdd }: {
  id: ColumnId; title: string; tasks: Task[]; onOpen: (id: string) => void; onAdd: (col: ColumnId) => void;
}) {
    ...
    <div className="w-[300px] shrink-0 flex flex-col">
      <div className="flex items-center justify-between px-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-secondary text-muted-foreground">{tasks.length}</span>
        </div>
        <button
          onClick={() => onAdd(id)}
      ...
      <div
        ...
        className={`flex-1 rounded-2xl p-2 min-h-[120px] transition-colors ${
          isOver ? "bg-[var(--neon-soft)] ring-1 ring-[var(--neon)]/40" : "bg-card/40 border border-border"
        }`}
      >
...
export function KanbanBoard({ search = "" }: { search?: string }) {
  const { tasks, activeBoardId, moveTask, reorderInColumn } = useStore();
  ...
  const grouped = useMemo(() => {
    const m: Record<ColumnId, Task[]> = { backlog: [], todo: [], in_progress: [], review: [], done: [] };
    for (const t of filtered) m[t.column].push(t);
    for (const k of Object.keys(m) as ColumnId[]) m[k].sort((a, b) => a.order - b.order);
    return m;
  }, [filtered]);
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    if (activeTask.column !== targetCol) {
      ...
      moveTask(activeTask.id, targetCol, newOrder);
      return;
    }
  ...
  return (
    <>
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={onDragStart} onDragEnd={onDragEnd}>
        ...
        >
          {COLUMNS.map((c) => (
            <Column key={c.id} id={c.id} title={c.title} tasks={grouped[c.id]} onOpen={setOpenId} onAdd={setCreateCol} />
          ))}
        </motion.div>
        <DragOverlay>
          {dragging && <TaskCard task={dragging} onOpen={() => {}} />}
        </DragOverlay>
      </DndContext>
      ...
      {(openId || createCol) && (
        <TaskModal
          ...
          onClose={() => { setOpenId(null); setCreateCol(null); }}
        />
      )}
    </>
  );
