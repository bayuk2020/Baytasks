...
export const Route = createFileRoute("/_app/")({
  head: () => ({ meta: [{ title: "Dashboard — BayTasks" }, { name: "description", content: "Your productivity command center." }] }),
  component: Dashboard,
});
...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const overdue = myTasks.filter((t) => t.dueAt && t.dueAt < Date.now() && t.column !== "done").length;
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-end justify-between flex-wrap gap-3">
        ...
        <Link to="/board" className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--gradient-neon)] text-primary-foreground text-sm font-medium neon-ring hover:scale-[1.02] transition">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          {upcoming.length === 0 ? (
            <div className="py-10 text-center text-sm text-muted-foreground">Nothing scheduled. Calm seas ahead.</div>
          ) : (
              ...
              {upcoming.map((t) => {
                ...
                return (
                  <li key={t.id} className="py-3 flex items-center justify-between">
                    ...
                    <div className={`text-xs ${overdue ? "text-[var(--priority-urgent)]" : "text-muted-foreground"}`}>
                      {t.dueAt ? new Date(t.dueAt).toLocaleString(undefined, { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : ""}
                    </div>
                  </li>
                );
              })}
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          <ul className="space-y-2">
            {boards.map((b) => {
              ...
              return (
                <li key={b.id} className="flex items-center justify-between rounded-lg border border-border bg-secondary/40 px-3 py-2 hover-lift">
                  <span className="flex items-center gap-2 text-sm"><span>{b.emoji}</span>{b.name}</span>
